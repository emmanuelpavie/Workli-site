...
export default function WorkliLanding() {
  ...
  // סדר RTL + תמונות זמניות + סטטוס זמינות
  const works = [
            ...
            <div className="grid grid-cols-3 items-center h-full gap-4">
              ...
              <div className="flex items-center gap-4 justify-start">
                <a href="#" className="font-semibold text-xl tracking-tight">
                  Workli
                  <span className="ml-2 align-middle text-[10px] text-white bg-gray-900 rounded px-1.5 py-0.5 opacity-80">
                    BETA
                  </span>
                </a>
                ...
                <div className="relative hidden md:block">
                  <button
                    ...
                    className="group inline-flex items-center gap-2 text-sm px-0 py-2 border-0 bg-transparent hover:opacity-80"
                  >
              ...
              <nav className="hidden lg:flex items-center justify-center gap-6 text-sm font-medium">
                ...
                <a href="#how" className="hover:text-gray-700">
                  איך זה עובד
                </a>
              ...
              <div className="hidden lg:flex items-center gap-3 justify-end">
                ...
                <a
                  className="rounded-2xl px-4 py-2 text-sm font-semibold bg-cyan-500 text-white hover:bg-cyan-600 transition shadow"
                  href="#signup"
                >
                  הרשמה
                </a>
        ...
        <div className="relative">
          {canRight && (
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 z-50 bg-white shadow rounded-full p-2 hover:shadow-md pointer-events-auto"
              aria-label="הבא"
            >
          ...
          {canLeft && (
            <button
              ...
              className="absolute left-2 top-1/2 -translate-y-1/2 z-50 bg-white shadow rounded-full p-2 hover:shadow-md pointer-events-auto"
              aria-label="הקודם"
            >
          ...
          <div
            dir="ltr"
            ref={scrollRef}
            onWheel={() => setTimeout(updateArrows, 0)}
            className="flex gap-6 overflow-x-auto scroll-smooth snap-x snap-mandatory py-4 no-scrollbar relative z-10"
          >
            {works.map((w) => (
              <div
                key={w.title}
                data-card
                className={`relative rounded-2xl overflow-hidden shadow-lg w-[360px] h-96 flex-shrink-0 group snap-start ${
                  w.disabled ? "bg-gray-100 grayscale" : "bg-gradient-to-br from-cyan-200 to-cyan-400"
                }`}
              >
                {/* תמונת רקע חצי שקופה */}
                <div
                  className="absolute inset-0 bg-cover bg-center opacity-40 group-hover:opacity-50 transition pointer-events-none"
                ...
                <div className="relative z-10 flex flex-col justify-between h-full p-6" dir="rtl">
                  ...
                  <div className={`mt-auto space-y-4 ${w.disabled ? "opacity-80" : ""}`}>
                    <div className="flex items-center justify-between gap-3">
                      {/* קופון מחיר */}
                      <span
                        className={`px-3 py-2 rounded-lg border border-dashed text-sm font-medium ${
                          w.disabled
                            ? "bg-white/80 border-gray-300 text-gray-500"
                            : "bg-cyan-50 border-cyan-400 text-cyan-700 font-semibold shadow ring-1 ring-cyan-200"
                        }`}
                      >
                        החל מ־₪250 למ"ר
                      </span>
                    ...
                    {/* כפתור הזמנה */}
                    <button
                      className={`w-full rounded-xl font-semibold py-2 transition relative overflow-hidden ${
                        w.disabled
                          ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                          : "text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 shadow-lg shadow-cyan-500/40 ring-1 ring-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-400"
                      }`}
                    >
                      הזמן עכשיו
                      {w.disabled && (
                        <span className="absolute inset-0 flex items-center justify-center text-sm text-gray-700 bg-white/80 opacity-0 group-hover:opacity-100 transition">
                          בהרצה – יהיה זמין בהמשך
                        </span>
                      )}
                    </button>
                  </div>
